...
def calculate_control(robot_x, robot_y, robot_a, goal_x, goal_y, alpha, beta, v_max, w_max):
    ...
    #
    alpha_0 = math.atan2(goal_y - robot_y, goal_x - robot_x)
    e_alpha = alpha_0 - robot_a
    v = v_max * np.exp(- (e_alpha**2) / alpha)
    w = w_max * (2 / (1 + np.exp(-e_alpha / beta)) -1)
    return [v,w]
...
def follow_path(path, alpha, beta, v_max, w_max):
    ...
    while numpy.linalg.norm(path[-1] - Pr) > 0.1 and not rospy.is_shutsown():
